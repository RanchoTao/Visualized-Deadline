...
import { createAIReviewExport, formatAIReviewExportFilename } from '../utils/aiReviewExport';
...
// weekly, monthly, or manual log context for AI-assisted reflection. No AI/API is called.
export function ActivityLog({ tasks, limit = Number.POSITIVE_INFINITY, title = '归档日志', onDelete, onReviewNoteChange }: ActivityLogProps) {
  ...
  function exportReviewContext() {
    if (archivedTasks.length === 0) {
      setExportStatus('还没有可导出的归档记录。');
      return;
    }

    const envelope = createAIReviewExport(archivedTasks);
    downloadJson(formatAIReviewExportFilename(new Date(envelope.exportedAt)), JSON.stringify(envelope, null, 2));
    setExportStatus(`已导出 ${envelope.data.summary.totalLogs} 条复盘素材，文件仅在本地生成。`);
  }
  ...
  return (
    <section className="rounded-[2rem] border border-white/70 bg-white/75 p-5 shadow-xl shadow-slate-200/60 backdrop-blur">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <p className="text-sm font-semibold text-slate-500">生命日志</p>
          <h2 className="text-2xl font-semibold text-slate-950">{title}</h2>
          <p className="mt-1 text-xs leading-5 text-slate-400">可将本地归档整理成结构化复盘素材；当前不会调用或上传到任何 AI 服务。</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button type="button" onClick={exportReviewContext} disabled={archivedTasks.length === 0} className="rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-slate-600 shadow-sm ring-1 ring-slate-200/80 hover:bg-slate-50 disabled:cursor-not-allowed disabled:text-slate-300">导出 AI 复盘素材</button>
        </div>
      </div>
      {exportStatus ? <p className="mt-3 rounded-2xl bg-sky-50 px-4 py-2 text-xs text-sky-700 ring-1 ring-sky-100">{exportStatus}</p> : null}
