...
import { clampProgress, getActivityTypeLabel } from '../utils/taskScoring';
    ...
    <form onSubmit={handleSubmit} className="space-y-4 rounded-[2rem] border border-white/70 bg-white/80 p-5 shadow-xl shadow-slate-200/60 backdrop-blur">
      <div>
        ...
        <input
          ...
          className="mt-2 w-full rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 outline-none transition focus:border-sky-200 focus:ring-4 focus:ring-sky-100/70"
      ...
      <div>
        ...
        <textarea
          ...
          className="mt-2 min-h-24 w-full rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 outline-none transition focus:border-sky-200 focus:ring-4 focus:ring-sky-100/70"
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <div>
          <label className="text-sm font-medium text-slate-600" htmlFor="importance">
            重要性（1-10）
          </label>
          <input
            id="importance"
        ...
        <div>
          <label className="text-sm font-medium text-slate-600" htmlFor="deadline">
            截止时间
          </label>
          <input
            id="deadline"
        ...
        <div>
          <label className="text-sm font-medium text-slate-600" htmlFor="progress">
            当前进度（0-100）
          </label>
          <input
            id="progress"
        ...
        <div>
          <label className="text-sm font-medium text-slate-600" htmlFor="activityType">
            活动类型
          </label>
          <select
            id="activityType"
            value={values.activityType}
            onChange={(event) => setValues({ ...values, activityType: event.target.value as ActivityType })}
            className="mt-2 w-full rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 outline-none transition focus:border-sky-200 focus:ring-4 focus:ring-sky-100/70"
          >
      ...
      {task ? (
        <div>
          <label className="text-sm font-medium text-slate-600" htmlFor="lifecycleStatus">
            状态
          </label>
          <select
            id="lifecycleStatus"
            value={values.lifecycleStatus}
            onChange={(event) => setValues({ ...values, lifecycleStatus: event.target.value as LifecycleStatus })}
            className="mt-2 w-full rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 outline-none transition focus:border-sky-200 focus:ring-4 focus:ring-sky-100/70"
          >
